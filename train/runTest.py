...
path = '../data/model_data05_exp50.dms'
...
for si in range(1):
    ...
    for i, data in enumerate(testLoader, 0):
        ...
        log(lf, "    pressure:  abs. difference, ratio: %f , %f " % (
        np.sum(np.abs(outputs_cpu[0] - targets_cpu[0])), lossPer_p.item()))
        log(lf, "    velocity:  abs. difference, ratio: %f , %f " % (
        np.sum(np.abs(outputs_cpu[1] - targets_cpu[1])) + np.sum(np.abs(outputs_cpu[2] - targets_cpu[2])),
        lossPer_v.item()))
        log(lf, "    aggregate: abs. difference, ratio: %f , %f " % (
        ...
        utils.imageOut("%04d" % (i), outputs_cpu, targets_cpu, normalize=False,
